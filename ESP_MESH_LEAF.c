...
//static const uint8_t bssid[6] = { 0xb0, 0x39, 0x56, 0x22, 0x26, 0x25 };

static const char *MESH_TAG = "mesh_main";
static const uint8_t MESH_ID[6] = { 0x77, 0x77, 0x77, 0x77, 0x77, 0x77};

static const uint8_t P_MESH_ID[6] = { 0xbc, 0xdd, 0xc2, 0xcb, 0x87, 0x51 };

static uint8_t tx_buf[TX_SIZE] = {"Message from LEAF to ROOT !!!, Anyway Welcome to ESP-MESH"};
...
void esp_mesh_comm_p2p_start(void)
{
	...
	mesh_data_t dat;
    	dat.data = tx_buf;
    	dat.size = sizeof(tx_buf);
    	dat.proto = MESH_PROTO_BIN;
    	dat.tos = MESH_TOS_P2P;
	
		err = esp_mesh_send(&P_MESH_ID, &dat, MESH_DATA_P2P, NULL, 0); // P_MESH_ID is working
		err = esp_mesh_send(NULL, &dat, MESH_DATA_P2P, NULL, 0);	// @param_1 NULL is working	
...
/*	wifi_config_t wifi_config = {
	...
	/* memcpy(&parent.sta.ssid, "Cognier-2", sizeof("Cognier-2"));
	memcpy(&parent.sta.password, "bravia452", sizeof("bravia452"));
	parent.sta.bssid_set = 0;			
			//memcpy(&parent.sta.bssid, bssid, 6);
	parent.sta.channel = 3; 
			
	esp_mesh_fix_root(true);
	esp_mesh_set_self_organized(0, 0);
	esp_mesh_set_parent(&parent, &P_MESH_ID, MESH_LEAF, 2);*/
